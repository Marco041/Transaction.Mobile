...
@Composable
fun SavingAccountEdit(id: Int,
                      filtersAppBar: (AppBarState?) -> Unit,
                      viewModel: SavingAccountEditViewModel) {
    LaunchedEffect(key1 = true) {
        filtersAppBar(null)
        viewModel.loadSavingAccount(id)
    }
...
@Composable
fun FormButton(viewModel: SavingAccountEditViewModel) {
    ...
    Column(Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally) {
        Row() {
            Button(
                onClick = {
                    composableScope.launch {
                        val result = viewModel.editSavingAccount()
            ...
            Button(
                onClick = {
                    composableScope.launch {
                        val result = viewModel.deleteSavingAccount()
                        viewModel.setSaveResult(result)
                        viewModel.setOpenSnackbar(true)
